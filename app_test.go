...
func TestApp(t *testing.T) {
	...
	testCases := []*ApiTestCase{
		...
		&ApiTestCase{
			...
			Method:         "POST",
			Body:           `{"article":{"title":"How to write golang tests", "description":"I have problem with mongodb mocking", "body":"Any ideas how to write some intermidiate layer atop collection?", "tagList":["golang","testing", "gomock"]}}`,
			URL:            "{{APIURL}}/articles",
			TokenName:      "token1",
			ResponseStatus: 201,
			Expected: func() interface{} {
				return &struct {
					...
					Article: TestArticle{
						...
						Description: "I have problem with mongodb mocking",
						CreatedAt:   FakeTime{true},
						UpdatedAt:   FakeTime{true},
	...
	for _, item := range testCases {
		ok := t.Run(item.Name, func(t *testing.T) {
			...
			if err != nil {
				t.Fatalf("cant read response body: %s", err)
			}
